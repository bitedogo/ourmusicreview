...
export function FindPasswordModal({ onClose }: FindPasswordModalProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      onClose();
      router.push(
        `/auth/reset-password?email=${encodeURIComponent(trimmedEmail)}&id=${encodeURIComponent(trimmedId)}`
      );
    } catch (submitError) {
      setError(
  ...
  return (
    ...
    >
      <div
        className="relative flex h-[250px] w-full max-w-[446px] flex-col rounded-[15px] border border-zinc-200 bg-white px-5 py-5 shadow-xl sm:h-[268px] sm:px-8 sm:py-6"
