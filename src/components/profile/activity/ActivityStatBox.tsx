...
export function ActivityStatBox({ label, count, href }: ActivityStatBoxProps) {
  ...
  if (!href) {
    return <div className={SHELL_CLASS}>{body}</div>;
  }

  return (
    <Link href={href} className={`block ${SHELL_CLASS}`}>
      {body}
    </Link>
  );
}
