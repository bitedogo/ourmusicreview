...
export function ItunesAlbumSearchPanel({
  ...
  if (isEmbedded) {
    return (
      <div>
        <label className="mb-1 block text-xs font-semibold text-[var(--color-text-secondary)]">앨범 검색</label>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-muted)] focus:border-zinc-400 focus:ring-2 focus:ring-zinc-200"
          />
          <button
            type="button"
          ...
          <div className="mt-3 max-h-40 overflow-auto rounded-lg border border-zinc-200">
            {picker.artists.map((artist) => (
              ...
              >
                ...
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="shrink-0 text-[var(--color-text-muted)]">
                  <path
                    d="M9 18L15 12L9 6"
                    stroke="currentColor"
                    strokeWidth="2"
          ...
          <div className="mt-3">
            <div className="mb-2 flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-[11px] font-medium text-[var(--color-text-secondary)] transition hover:bg-zinc-50"
              >
            ...
            {picker.isLoadingAlbums ? (
              <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-6 text-center text-xs text-[var(--color-text-secondary)]">
                앨범 목록을 불러오는 중...
              </div>
                ...
                {picker.albums.map((album) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[11px] text-[var(--color-text-secondary)]">{album.artistName}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        {!picker.isSearchingArtists &&
          ...
          picker.searchQuery && (
            <p className="mt-2 text-xs text-[var(--color-text-secondary)]">검색 결과가 없습니다.</p>
          )}
      </div>
    );
  }
